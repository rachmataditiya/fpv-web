...
**Combat** (`game/weapon.ts`, `game/barrels.ts`, `game/pickups.ts`): hitscan resolved in the physics tick. The player arsenal is config-driven (`WEAPON_CONFIGS`): **blaster** (tap edge or hold to auto-fire ~9 rps, heat-based spread), **burst** (3-round burst per trigger), **railgun** (hold to charge 0.8 s, release to fire, 2 s cooldown). `Weapon.setTriggerHeld()` carries the trigger level each tick; `setConfig()` swaps it (keys 1/2/3, Q cycles; gamepad RB / DJI left-center switch = next weapon). Pickups (`PickupField`, BSP maps only) swap the weapon on contact and respawn in place after 20 s. The aim direction is the **FPV camera** (body forward rotated by the uptilt setting), so shots land on the HUD crosshair — firing along raw body-forward is a bug, not a simplification. World `sweep` blocks shots; barrels and pickups are sphere/contact objects on real floors via `floorAt`, seeded/deterministic. FX (`render/fx.ts`: tracer/muzzle/explosion/impact pools) and SFX (`audio/sfx.ts`, layered procedural WebAudio) are pooled/lazy.

**GLTF assets** (`render/modelLoader.ts`, Kenney CC0 in `public/assets/models/` + `fx/`): every consumer keeps its PROCEDURAL mesh as the instant visual and the GLB swaps in async when it loads — a 404 must never break the game (that contract is tested). Kenney GLBs reference an EXTERNAL `Textures/colormap.png` per pack, so each pack lives in its own subfolder (`arena/`, `blaster/`) or the texture 404s and everything renders white. The rigged soldier (`render/soldierGltf.ts`) does NOT use the pack's AnimationMixer clips — they mix two root-orientation conventions ('walk' lays the model flat) — instead we puppet the six named bones directly (legs/arms/torso), like the procedural soldier's pivots. Template preloads at boot; `createSoldierInstance()` is a sync SkeletonUtils clone (null → BotManager falls back to procedural).
